...
CREATE TABLE IF NOT EXISTS Dates (
  ...
  CONSTRAINT CheckValues CHECK (
        (type = 'e' AND
            precision IS NOT NULL AND precision IN ('d', 'h', 'm') AND
            diff      IS NOT NULL AND
            diffType  IS NOT NULL AND diffType IN ('d', 'h', 'm') AND
            date2     IS NULL

        ) !=  -- Effectively just an XOR

        (type = 'e' AND
            precision IS NULL AND
            diff      IS NULL AND
            diffType  IS NULL AND
            date2     IS NOT NULL
        ))
);
...
CREATE TABLE IF NOT EXISTS ChangeLog (
  cio       INTEGER      NOT NULL PRIMARY KEY,
  eid       INTEGER      NOT NULL FOREIGN KEY REFERENCES Events(eid),
  desc      VARCHAR(64k) NOT NULL,
